...
type PromiseHandler = (data: any) => Promise<any>

export class EventEmitter {
  eventHandlers = new Map<string, (EventHandler | PromiseHandler)[]>()
  ws: WebSocket
  ...
  handleEvent(eventName: string, data: any) {
    ...
    let hasErrors = false
    const handlerHandler = (handler: typeof handlers[number], index: number) => {
      try {
    ...
    const hasSomePromise = handlers.some((handler) => handler instanceof Promise)
    if (hasSomePromise) {
      return Promise.all(handlers.map(handlerHandler))
        .then(() => throwIfErrors())
    } else {
      handlers.forEach(handlerHandler)
      throwIfErrors()
    }
  }
